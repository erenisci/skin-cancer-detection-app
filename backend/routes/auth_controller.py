...
from internal.email.verification_code import (generate_code,
...
from internal.models.user import ForgotPasswordRequest, UserLogin, UserSignUp
...
async def login(user: UserLogin):
    ...
    access_token_obj = AccessToken(
        ...
        expires_at=datetime.now(
            timezone.utc) + timedelta(minutes=conf["access_token_expire_minutes"]),
        is_active=True
    )
    ...
    refresh_token_obj = RefreshToken(
        ...
        created_at=datetime.now(timezone.utc),
        expires_at=datetime.now(
            timezone.utc) + timedelta(days=conf["refresh_token_expire_days"]),
        is_active=True
    )
...
async def refresh_access_token(refresh_token: str = Body(..., embed=True)):
    ...
    if not refresh_token_data.get("is_active", True):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Inactive refresh token")
