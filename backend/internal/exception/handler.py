...
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    return JSONResponse(
        ...
        content=create_api_error(
            request, exc.detail, ErrorCodes.HTTP_EXCEPTION)
    )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        status_code=422,
        content=create_api_error(
            request, "Validation Error", ErrorCodes.VALIDATION_ERROR, details=errors)
    )
...
async def generic_exception_handler(request: Request, exc: Exception):
    return JSONResponse(
        status_code=500,
        content=create_api_error(
            request, "Internal Server Error", ErrorCodes.INTERNAL_SERVER_ERROR)
    )
