...
import numpy as np
import pandas as pd
from detection.processing.constants import (CLASS_COLORS, CLASS_MAPPING,
                                            IMAGES_PATH, LABELS_PATH,
                                            MASKS_PATH, METADATA_FILE,
                                            OUTPUT_PATH)
...
for i, (_, row) in enumerate(df.iterrows()):
    ...
    with open(label_file, "w") as f:
        for x, y, bw, bh in bounding_boxes:
            ...
            cv2.putText(image, list(CLASS_MAPPING.keys())[
                        class_id], (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, CLASS_COLORS[yolo_class], 2)
